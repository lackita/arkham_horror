...
(defn make
  ([name stats] (make board/board name stats))
  ([board name {speed :speed fight :fight lore :lore}]
...
(defn attack [investigator]
  (dosync (board/update-status! (-> @investigator :board :status)
                                "Roll: "
                                "Attack"
                                '(accept-roll))))
